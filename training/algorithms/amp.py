...
def parametric_walk_dataset(default_joint_pos, device,
    ...
    def feats(q, v, ph):
        ...
        # For parametric walk, we just assume feet are near ground (0 clearance)
        # or we could synthesize a simple foot clearance.
        # Let's synthesize a simple clearance: 0.05 max height during swing.
        # swing happens when sin(phase) is positive for left leg, etc.
        pL = 2 * np.pi * ph
        pR = 2 * np.pi * (ph + 0.5)
        fL = 0.05 * np.maximum(0, np.sin(pL))
        fR = 0.05 * np.maximum(0, np.sin(pR))
        foot_clear = np.stack([fL, fR], axis=1).astype(np.float32)
        return build_amp_obs(h, proj_g, ang, q, v, foot_clear)

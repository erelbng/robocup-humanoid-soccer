...
class MuJoCoEvaluator:
    ...
    def _add_field_and_ball_to_spec(self, spec):
        """Programmatically add field carpet, lines, goals, and ball to a
        K1-rooted MjSpec. Centralised here so the eval and debug paths can
        diverge if needed without duplicating geometry.
        """
        import math as _m
        mj = self.mj
        wb = spec.worldbody
        f = self.field_info
        hl = f.get("half_length", 4.5)
        hw = f.get("half_width", 3.0)
        tl = f.get("total_length", 11.0)
        tw = f.get("total_width", 8.0)
        gw = f.get("goal_width", 2.6) / 2
        gh = f.get("goal_height", 0.8)
        gd = f.get("goal_depth", 0.6)
        ...
        for sign in (1, -1):
            for name, sz, pos in [
                (f"touchline_{'p' if sign>0 else 'n'}", (hl, 0.025, 0.001),
                    (0, sign * hw, lz)),
                (f"goalline_{'p' if sign>0 else 'n'}", (0.025, hw, 0.001),
                    (sign * hl, 0, lz)),
            ]:
                g = wb.add_geom()
                g.name = name
        ...
        for sign in (1, -1):
            ...
            for ysign in (1, -1):
                ...
                g = wb.add_geom()
                g.name = f"post_{side}_{ys}"
                g.type = mj.mjtGeom.mjGEOM_CYLINDER
                g.size = [0.05, gh / 2, 0]
                g.pos = [gx + sign * gd / 2, ysign * gw, gh / 2]
                g.rgba = [0.95, 0.95, 0.95, 1]
            g = wb.add_geom()
            g.name = f"crossbar_{side}"
            g.type = mj.mjtGeom.mjGEOM_BOX
            g.size = [0.05, gw + 0.05, 0.05]
            g.pos = [gx + sign * gd / 2, 0, gh]
            g.rgba = [0.95, 0.95, 0.95, 1]
